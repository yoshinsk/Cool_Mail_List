...
- 送信者/SMTP設定の編集、削除/無効化、SMTP接続チェック
...
送信者/SMTP管理では、登録済み送信者ごとにSMTPホスト、ポート、暗号化、認証ID、パスワード、上限値、DKIM方針を編集できます。SMTPチェックはメールを送信せず、SMTPサーバへの接続と認証だけを確認します。通常、587番はTLS、465番はSSLを選択します。削除時にキャンペーンやキューの使用履歴がある場合は、履歴を守るため物理削除せず無効化します。
